...
def experiment():
    ...
    def split_across_devices(images, labels, num_devices):
        ...
        split_images = images.reshape((num_devices, -1, *images.shape[2:]))
        logger.info(f"Split images shape: {split_images.shape}")
        split_labels = labels.reshape((num_devices, -1, *labels.shape[2:]))
    ...
    init_params = jax.tree_util.tree_map(lambda x: jax.device_put_sharded([x] * num_devices, devices), init_params)
    ...
    for epoch in range(1, FLAGS.epochs + 1):
        ...
        for _ in range(num_batches):
          ...
          t = time.time()
          if FLAGS.dpsgd:
            opt_state, total_grad_norm, total_aug_norms = partial(private_update, l2_norm_clip=l2_norm_clip, key=key, i=next(itercount))(opt_state, shape_as_image(*next_batch, dummy_dim=True, augmult=FLAGS.augmult, flatten_augmult=False), add_params)
        ...
        # log_memory_usage(logger, handle)
        train_acc, _, _ = accuracy(params, shape_as_image(train_images, train_labels, augmult=0), splits=4)
