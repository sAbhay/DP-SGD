...
GetVelocityFn = Callable[[OptimizerState], Params]
...
def optimizer(opt_maker: Callable[...,
  ...
  def tree_opt_maker(*args, **kwargs):
    init, update, get_params, get_velocity, set_params = opt_maker(*args, **kwargs)
    ...
    @functools.wraps(get_velocity)
    def tree_get_velocity(opt_state):
        states_flat, tree, subtrees = opt_state
        states = map(tree_unflatten, subtrees, states_flat)
        velocity = map(get_velocity, states)
        return tree_unflatten(tree, velocity)
    ...
    return Optimizer(tree_init, tree_update, tree_get_params, tree_get_velocity, tree_set_params)
