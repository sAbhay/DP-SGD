      ...
      dict(
          config=dict(
              ...
              training=dict(
                  batch_size=dict(
                      init_value=4096,
                      per_device_per_step=16, # augmult * per_device_per_step = 64 for compatability with jagupard GPUs
                  ...
                  dp=dict(
                      target_delta=1e-5,
                      clipping_norm=0.75,  # float('inf') or None to deactivate
                      stop_training_at_epsilon=0.5,  # None,
